...
for t = 1 : nt
    ...
    cont_y = nan(nz+2, n_p);
    cont_z = zeros(nz+2, n_p);  % 値は使わないが surface 用に必要
    ...
    if date >= min_date && date <= max_date
        for p = 1 : n_p
            ...
            % 海面と海底の深さ（Z方向）を作る
            cont_y(1,p) = 0;       % 海面（上）
            cont_y(nz+2,p) = -h;   % 海底（下）

            % 間を線形で補完（層っぽく見せる）
            for k = 1 : nz
                cont_y(k+1,p) = -h * (k / (nz+1));
            end
        end

        % 単色で横断図描画
        [h_surf, h_annot] = createmap_singlecolor(cont_x, cont_y, cont_z, ...
            date_str, font_size, y_range, xsize, ysize, title);

        if fig_save == 1
            fname = strcat('depthsection', datestr(date,'_yyyymmddHHMM'));
            hgexport(gcf, fullfile(out_dirstr, [fname '.png']), hgexport('factorystyle'), 'Format', 'png');
        end
    end
end
...
function [h_surf, h_annot] = createmap_singlecolor(cont_x, cont_y, cont_z, ...
    date_str, font_size, y_range, xsize, ysize, title_str)

figure1 = figure('Color',[1 1 1], ...
                 'Position',[100 100 xsize ysize]);

axes1 = axes('Parent',figure1, ...
             'YTick', y_range(1):5:y_range(2), ...
             'YDir', 'reverse', ...
             'FontSize', 10, ...
             'FontName', 'Arial', ...
             'Box', 'on');
...
%%ここ水深で単色で塗るときだけ
% 単色で塗るためのカラーデータ作成（cont_zは使わず全体を1にする）
color_data = ones(size(cont_z));

h_surf = pcolor(cont_x, -cont_y, color_data);
shading flat;
colormap(gca, [0.7 0.8 1.0]); % 薄い水色の単色カラーマップ
colorbar off
